...
void VkGraphicsPipelineBuilder::DoBuild() {
  ...
  // all we will do there are fixed states
  ...
  viewport.x = 0.0f;
  viewport.y = 0.0f;
  viewport.width = (float) pCtx->swapChainExtent.width;
  viewport.height = (float) pCtx->swapChainExtent.height;
  viewport.minDepth = 0.0f;
  viewport.maxDepth = 1.0f;

  VkRect2D scissor{};
  scissor.offset = {0, 0};
  scissor.extent = pCtx->swapChainExtent;

  // but there are some excludes of fixed states:
  ...
  // options for blending different framebuffers
  ...
  pipelineInfo.layout = pCtx->pipelineLayout;

  pipelineInfo.renderPass = pCtx->renderPass;
...
void VkGraphicsPipelineBuilder::DoDestroy() {
  vkDestroyPipeline(pCtx->device, pCtx->graphicsPipeline, nullptr);
  vkDestroyPipelineLayout(pCtx->device, pCtx->pipelineLayout, nullptr);
}

VkShaderModule VkGraphicsPipelineBuilder::CreateShaderModule(
